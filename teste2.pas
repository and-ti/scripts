...
    FIsMinimizing: Boolean;
    FIsClosing: Boolean; // NOVA FLAG: Indica que uma operação de fechar está em andamento
...
  FIsDestroying := False;
  FIsMinimizing := False;
  FIsClosing := False; // Inicializa a nova flag
...
      AnimPos.StartValue := FMainFormHeight;
      AnimPos.StopValue := 0;
      AnimPos.Duration := 0.3;
      AnimPos.AnimationType := TAnimationType.Out;
      AnimPos.Interpolation := TInterpolationType.Quadratic;
      // RE-CORREÇÃO: Garante que o TagObject seja atribuído para o OnFinish funcionar
...
    else
    begin
      // CORREÇÃO: Verifica se a ativação é por causa de uma minimização OU fechamento
      if FIsMinimizing or FIsClosing then
      begin
...
          UpdateHeaderUI(nil);
        ...
        // CORREÇÃO: Reseta a flag após a conclusão da operação de fechamento.
        if FIsClosing then
          FIsClosing := False;
      end;
    end;
...
    Exit;
  ...
  // CORREÇÃO: Sinaliza que a operação de fechar começou.
  FIsClosing := True;
